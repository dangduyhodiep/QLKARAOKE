...
//			if (adapterMd.checkTenPhong(txtTen.getText().toString())) {
//			JOptionPane.showMessageDialog(null, "Tên phòng đã tồn tại. Vui lòng chọn tên khác!");
//			} else {
//			// Room name is unique, proceed with adding the room
//			adapterMd.ThemPhong(txtTen.getText().toString(), chonloaiphong,
//			txtGia.getText().toString(), txtNote.getText().toString());
//			adapterCtr = new KetnoiKaraoke();
//			// Load all room data
//			tblPhong.setModel(adapterCtr.timKiemPhong(6));
//			// Set text on the label notification!
//			}
...
//			}
//		});
//		cmbLp.setModel(new DefaultComboBoxModel(new String[] {"--chọn Phòng--","phòng Thường", "phòng Vip" }));
		...
		btnThem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (!quyen.equals("1"))
					...
					//check if the room name already exists
					//
/**					adapterMd.CheckTenPhong(txtTen.getText().toString(), chonloaiphong, txtGia.getText().toString(),
							txtNote.getText().toString());
						JOptionPane.showMessageDialog(null, "Tên Phòng Đã Tồn Tại. Vui Lòng Chọn Tên Khác!");
					}
*/					//
